...
interface NewsCardProps {
  news: NewsItem; // A single news object (title, date, summary, category, etc.)
}

export default function NewsCard({ news }: NewsCardProps) {
  // Selects the right icon depending on the news priority
  const getPriorityIcon = (priority: string) => {
    switch (priority) {
      ...
      default: 
        return null; // "Normale" → no extra icon
    }
  };

  // Chooses the left border color depending on the news priority
  ...
  return (
    <div 
      className={`bg-white rounded-lg shadow-md border-l-4 
                  ${getPriorityBorder(news.priority)} 
                  p-6 hover:shadow-lg transition-shadow`}
    >
      {/* Header (icon + category + priority) */}
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-3">
          {/* News main icon with background color */}
          ...
          <div>
            ...
            {/* Priority (only shown if "Haute" or "Moyenne") */}
